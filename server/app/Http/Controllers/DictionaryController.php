...
class DictionaryController extends Controller
{
    ...
    public function details_dictionary($dictionary_id){
        ...
        $wordtype_dictionary = DB::table('tbl_wordtype')->where('wordtype_status','0')->orderby('wordtype_id','desc')->get(); 
        $alphabet_dictionary = DB::table('tbl_alphabet')->where('alphabet_status','0')->orderby('alphabet_id','desc')->get(); 

        $details_dictionary = DB::table('tbl_dictionary')
        ->join('tbl_wordtype','tbl_wordtype.wordtype_id','=','tbl_dictionary.wordtype_id')
        ->join('tbl_alphabet','tbl_alphabet.alphabet_id','=','tbl_dictionary.alphabet_id')
        ->where('tbl_dictionary.dictionary_id',$dictionary_id)->get();

        foreach($details_dictionary as $key =>$value)
        {
            $wordtype_id = $value->wordtype_id;
        }
        $related_dictionary = DB::table('tbl_dictionary')
        ->join('tbl_wordtype','tbl_wordtype.wordtype_id','=','tbl_dictionary.wordtype_id')
        ->join('tbl_alphabet','tbl_alphabet.alphabet_id','=','tbl_dictionary.alphabet_id')
        ->where('tbl_wordtype.wordtype_id',$wordtype_id)->whereNotIn('tbl_dictionary.dictionary_id',[$dictionary_id])->get();

        // $related_dictionary = DB::table('tbl_dictionary')
        // ->join('tbl_wordtype','tbl_wordtype.wordtype_id','=','tbl_dictionary.wordtype_id')
        // ->join('tbl_alphabet','tbl_alphabet.alphabet_id','=','tbl_alphabet.alphabet_id')
        // ->where('tbl_wordtype.wordtype_id',$wordtype_id)
        // ->whereNotIn('tbl_dictionary.dictionary_id',[$dictionary_id])
        // ->get();

        return view('pages.show_details')
        ->with('wordtype_dictionary',$wordtype_dictionary)
        ->with('alphabet_dictionary',$alphabet_dictionary)
